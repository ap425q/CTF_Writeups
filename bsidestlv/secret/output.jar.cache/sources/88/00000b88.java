...
/* loaded from: output.jar:com/google/android/material/color/Cam16.class */
final class Cam16 {
    ...
    static Cam16 fromIntInViewingConditions(int i, ViewingConditions viewingConditions) {
        ...
        double d6 = f22;
        return new Cam16(f, pow6, pow5, c * sqrt * (aw + 4.0f) * flRoot, flRoot2, sqrt2 * 50.0f, f23, log1p * ((float) Math.cos(d6)), log1p * ((float) Math.sin(d6)));
    }
    ...
    private static Cam16 fromJchInViewingConditions(float f, float f2, float f3, ViewingConditions viewingConditions) {
        ...
        return new Cam16(f3, f2, f, c * sqrt * (aw + 4.0f) * flRoot, flRoot2, sqrt2 * 50.0f, f4, log1p * ((float) Math.cos(d2)), log1p * ((float) Math.sin(d2)));
    }
