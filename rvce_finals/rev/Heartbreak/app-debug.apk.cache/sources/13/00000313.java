...
public class Motion implements TypedValues {
    ...
    MotionKeyPosition getPositionKeyframe(int layoutWidth, int layoutHeight, float x, float y) {
        ...
        start.left = this.mStartMotionPath.f21x;
        start.top = this.mStartMotionPath.f22y;
        ...
        end.left = this.mEndMotionPath.f21x;
        end.top = this.mEndMotionPath.f22y;
    ...
    float getKeyFrameParameter(int type, float x, float y) {
        float dx = this.mEndMotionPath.f21x - this.mStartMotionPath.f21x;
        float dy = this.mEndMotionPath.f22y - this.mStartMotionPath.f22y;
        float startCenterX = this.mStartMotionPath.f21x + (this.mStartMotionPath.width / 2.0f);
        float startCenterY = this.mStartMotionPath.f22y + (this.mStartMotionPath.height / 2.0f);
    ...
    public void setup(int parentWidth, int parentHeight, float transitionDuration, long currentTime) {
        ...
        if (points[0].mPathMotionArc != -1) {
            ...
            for (int i9 = 0; i9 < size; i9++) {
                ...
                values[i9][0] = points[i9].f21x;
                values[i9][1] = points[i9].f22y;
            }
            ...
            case -1:
                ...
                return new DifferentialInterpolator() { // from class: androidx.constraintlayout.core.motion.Motion.1
                    ...
                    public float getVelocity() {
                        return (float) Easing.this.getDiff(this.f18mX);
                    }
                };
            default:
                return null;
    ...
    public boolean interpolate(MotionWidget child, float global_position, long time, KeyCache keyCache) {
        ...
        if (this.mSpline != null) {
            ...
            float float_l = this.mStartMotionPath.f21x + ((this.mEndMotionPath.f21x - this.mStartMotionPath.f21x) * position);
            float float_t = this.mStartMotionPath.f22y + ((this.mEndMotionPath.f22y - this.mStartMotionPath.f22y) * position);
    ...
    void getDpDt(float position, float locationX, float locationY, float[] mAnchorDpDt) {
        ...
        float dleft = this.mEndMotionPath.f21x - this.mStartMotionPath.f21x;
        float dTop = this.mEndMotionPath.f22y - this.mStartMotionPath.f22y;
