...
public class MaterialToolbar extends Toolbar {
    private static final int DEF_STYLE_RES = C0558R.style.Widget_MaterialComponents_Toolbar;
    ...
    public MaterialToolbar(Context context, AttributeSet attrs) {
        this(context, attrs, C0558R.attr.toolbarStyle);
    }

    public MaterialToolbar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(MaterialThemeOverlay.wrap(context, attrs, defStyleAttr, DEF_STYLE_RES), attrs, defStyleAttr);
        Context context2 = getContext();
        TypedArray a = ThemeEnforcement.obtainStyledAttributes(context2, attrs, C0558R.styleable.MaterialToolbar, defStyleAttr, DEF_STYLE_RES, new int[0]);
        if (a.hasValue(C0558R.styleable.MaterialToolbar_navigationIconTint)) {
            setNavigationIconTint(a.getColor(C0558R.styleable.MaterialToolbar_navigationIconTint, -1));
        }
        this.titleCentered = a.getBoolean(C0558R.styleable.MaterialToolbar_titleCentered, false);
        this.subtitleCentered = a.getBoolean(C0558R.styleable.MaterialToolbar_subtitleCentered, false);
        int index = a.getInt(C0558R.styleable.MaterialToolbar_logoScaleType, -1);
        if (index >= 0 && index < LOGO_SCALE_TYPE_ARRAY.length) {
            this.logoScaleType = LOGO_SCALE_TYPE_ARRAY[index];
        }
        if (a.hasValue(C0558R.styleable.MaterialToolbar_logoAdjustViewBounds)) {
            this.logoAdjustViewBounds = Boolean.valueOf(a.getBoolean(C0558R.styleable.MaterialToolbar_logoAdjustViewBounds, false));
        }
        a.recycle();
