...
public abstract class DispatchedTask<T> extends Task {
    ...
    public final void run() {
        ...
        try {
            ...
            oldValue$iv = ThreadContext.updateThreadContext(context$iv, countOrElement$iv);
            job = null;
            if (oldValue$iv != ThreadContext.NO_THREAD_ELEMENTS) {
                undispatchedCoroutine = CoroutineContext.updateUndispatchedCompletion(continuation, context$iv, oldValue$iv);
            } else {
            ...
            try {
                ...
                result = Result.m221constructorimpl(Unit.INSTANCE);
            } catch (Throwable th) {
                th = th;
                Result.Companion companion2 = Result.Companion;
                result = Result.m221constructorimpl(ResultKt.createFailure(th));
                handleFatalException(fatalException, Result.m224exceptionOrNullimpl(result));
            }
        }
        try {
            ...
            try {
                if (job != null && !job.isActive()) {
                    ...
                    if (Debug.getRECOVER_STACK_TRACES()) {
                        try {
                            if (continuation instanceof CoroutineStackFrame) {
                                ...
                                continuation.resumeWith(Result.m221constructorimpl(ResultKt.createFailure(cancellationException)));
                            }
                        } catch (Throwable th3) {
                            th = th3;
                    ...
                    continuation.resumeWith(Result.m221constructorimpl(ResultKt.createFailure(exception)));
                } else {
                    Result.Companion companion5 = Result.Companion;
                    continuation.resumeWith(Result.m221constructorimpl(getSuccessfulResult$kotlinx_coroutines_core(state)));
                }
    ...
    public final void handleFatalException(Throwable exception, Throwable finallyException) {
        ...
        if (exception != null && finallyException != null) {
            Exceptions.addSuppressed(exception, finallyException);
        }
