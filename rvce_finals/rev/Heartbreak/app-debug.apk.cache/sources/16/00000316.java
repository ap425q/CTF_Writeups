...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = startTimePoint.f21x + (startTimePoint.width / 2.0f);
        float startCenterY = startTimePoint.f22y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f21x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f22y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f21x = (int) ((startTimePoint.f21x + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f22y = (int) ((startTimePoint.f22y + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 0;
        this.f21x = (int) (((startTimePoint.f21x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f22y = (int) (((startTimePoint.f22y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    void initPolar(int parentWidth, int parentHeight, MotionKeyPosition c, MotionPaths s, MotionPaths e) {
        ...
        switch (c.mPositionType) {
            case 1:
                this.f21x = ((Float.isNaN(c.mPercentX) ? position : c.mPercentX) * (e.f21x - s.f21x)) + s.f21x;
                this.f22y = ((Float.isNaN(c.mPercentY) ? position : c.mPercentY) * (e.f22y - s.f22y)) + s.f22y;
                break;
            case 2:
                this.f21x = Float.isNaN(c.mPercentX) ? ((e.f21x - s.f21x) * position) + s.f21x : c.mPercentX * Math.min(scaleHeight, scaleWidth);
                this.f22y = Float.isNaN(c.mPercentY) ? ((e.f22y - s.f22y) * position) + s.f22y : c.mPercentY;
                break;
            default:
                this.f21x = ((Float.isNaN(c.mPercentX) ? position : c.mPercentX) * (e.f21x - s.f21x)) + s.f21x;
                this.f22y = ((Float.isNaN(c.mPercentY) ? position : c.mPercentY) * (e.f22y - s.f22y)) + s.f22y;
    ...
    public void setupRelative(Motion mc, MotionPaths relative) {
        double dx = ((this.f21x + (this.width / 2.0f)) - relative.f21x) - (relative.width / 2.0f);
        double dy = ((this.f22y + (this.height / 2.0f)) - relative.f22y) - (relative.height / 2.0f);
        this.mRelativeToController = mc;
        this.f21x = (float) Math.hypot(dy, dx);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f22y = (float) (Math.atan2(dy, dx) + 1.5707963267948966d);
        } else {
    ...
    void initScreen(int parentWidth, int parentHeight, MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        if (!Float.isNaN(c.mPercentX)) {
            this.f21x = (int) (c.mPercentX * ((int) (parentWidth - this.width)));
        }
        if (!Float.isNaN(c.mPercentY)) {
            float pathVectorX2 = (int) (parentHeight - this.height);
            this.f22y = (int) (c.mPercentY * pathVectorX2);
        }
    ...
    void initPath(MotionKeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f21x = (int) ((startTimePoint.f21x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f22y = (int) ((startTimePoint.f22y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (startTimePoint.width + (scaleX * scaleWidth));
        this.height = (int) (startTimePoint.height + (scaleY * scaleHeight));
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean diffx = diff(this.f21x, points.f21x);
        boolean diffy = diff(this.f22y, points.f22y);
        int c = 0 + 1;
    ...
    public void getCenter(double p, int[] toUse, double[] data, float[] point, double[] vdata, float[] velocity) {
        ...
        if (this.mRelativeToController != null) {
            ...
            float v_y3 = (float) ((ry - (radius * Math.cos(angle))) - (v_height / 2.0f));
            float dpos_x = (float) (v_x2 + (dradius * Math.sin(angle)) + (Math.cos(angle) * dangle2));
            dpos_y = (float) ((v_y2 - (dradius * Math.cos(angle))) + (Math.sin(angle) * dangle2));
            dangle = dpos_x;
            v_y = v_y3;
            v_x = v_x3;
        }
    ...
    public void setView(float position, MotionWidget view, int[] toUse, double[] data, double[] slope, double[] cycle) {
        ...
        float delta_path;
        double d;
        float v_x2 = this.f21x;
        float v_y3 = this.f22y;
        float v_width = this.width;
        ...
        if (this.mRelativeToController != null) {
            ...
            float dpos_x = (float) (drx + (d2 * Math.sin(angle)) + (radius * Math.cos(angle) * dangle));
            ...
            float path_rotate4 = path_rotate2;
            if (!Float.isNaN(path_rotate4)) {
                float dx = dv_x2 + (dv_width2 / 2.0f);
                float dy = dv_y3 + (dv_height3 / 2.0f);
                float rot2 = (float) (0.0f + path_rotate4 + Math.toDegrees(Math.atan2(dy, dx)));
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f21x, this.f22y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
