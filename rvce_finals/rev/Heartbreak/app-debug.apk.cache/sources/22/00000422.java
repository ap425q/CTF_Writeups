...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = startTimePoint.f57x + (startTimePoint.width / 2.0f);
        float startCenterY = startTimePoint.f58y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f57x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f58y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f57x = (int) ((startTimePoint.f57x + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f58y = (int) ((startTimePoint.f58y + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 0;
        this.f57x = (int) (((startTimePoint.f57x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f58y = (int) (((startTimePoint.f58y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    void initPolar(int parentWidth, int parentHeight, KeyPosition c, MotionPaths s, MotionPaths e) {
        ...
        switch (c.mPositionType) {
            case 1:
                this.f57x = ((Float.isNaN(c.mPercentX) ? position : c.mPercentX) * (e.f57x - s.f57x)) + s.f57x;
                this.f58y = ((Float.isNaN(c.mPercentY) ? position : c.mPercentY) * (e.f58y - s.f58y)) + s.f58y;
                break;
            case 2:
                this.f57x = Float.isNaN(c.mPercentX) ? ((e.f57x - s.f57x) * position) + s.f57x : c.mPercentX * Math.min(scaleHeight, scaleWidth);
                this.f58y = Float.isNaN(c.mPercentY) ? ((e.f58y - s.f58y) * position) + s.f58y : c.mPercentY;
                break;
            default:
                this.f57x = ((Float.isNaN(c.mPercentX) ? position : c.mPercentX) * (e.f57x - s.f57x)) + s.f57x;
                this.f58y = ((Float.isNaN(c.mPercentY) ? position : c.mPercentY) * (e.f58y - s.f58y)) + s.f58y;
    ...
    public void setupRelative(MotionController mc, MotionPaths relative) {
        double dx = ((this.f57x + (this.width / 2.0f)) - relative.f57x) - (relative.width / 2.0f);
        double dy = ((this.f58y + (this.height / 2.0f)) - relative.f58y) - (relative.height / 2.0f);
        this.mRelativeToController = mc;
        this.f57x = (float) Math.hypot(dy, dx);
        if (Float.isNaN(this.mRelativeAngle)) {
            this.f58y = (float) (Math.atan2(dy, dx) + 1.5707963267948966d);
        } else {
    ...
    void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        if (!Float.isNaN(c.mPercentY)) {
            ...
            this.f58y = (int) (c.mPercentY * pathVectorX2);
        }
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f57x = (int) ((startTimePoint.f57x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f58y = (int) ((startTimePoint.f58y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (startTimePoint.width + (scaleX * scaleWidth));
        this.height = (int) (startTimePoint.height + (scaleY * scaleHeight));
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        boolean diffx = diff(this.f57x, points.f57x);
        boolean diffy = diff(this.f58y, points.f58y);
        int c = 0 + 1;
    ...
    public void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        float delta_path;
        double d;
        float v_x2 = this.f57x;
        float v_y2 = this.f58y;
        float v_width = this.width;
        ...
        if (this.mRelativeToController != null) {
            ...
            float pos_x = (float) ((rx + (radius * Math.sin(v_y4))) - (v_width2 / 2.0f));
            float pos_y = (float) ((ry - (radius * Math.cos(v_y4))) - (v_height2 / 2.0f));
            v_height = v_height2;
            dangle = v_width2;
            float dpos_x = (float) (drx + (dradius * Math.sin(v_y4)) + (radius * Math.cos(v_y4) * dangle2));
            float dpos_y = (float) ((dry - (dradius * Math.cos(v_y4))) + (radius * Math.sin(v_y4) * dangle2));
            v_x = pos_x;
            if (slope.length < 2) {
                z2 = false;
                z = true;
            ...
            if (!Float.isNaN(angle)) {
                dv_y = dpos_y;
                float rot = (float) (angle + Math.toDegrees(Math.atan2(dpos_y, dpos_x)));
                view2 = view;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f57x, this.f58y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
