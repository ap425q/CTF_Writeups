...
class PathInterpolatorApi14 implements Interpolator {
    ...
    PathInterpolatorApi14(Path path) {
        ...
        this.f80mX = new float[numPoints];
        this.f81mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            float distance = (i * pathLength) / (numPoints - 1);
            pathMeasure.getPosTan(distance, position, null);
            this.f80mX[i] = position[0];
            this.f81mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f80mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f80mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f80mX[endIndex] - this.f80mX[startIndex];
        if (xRange == 0.0f) {
            return this.f81mY[startIndex];
        }
        float tInRange = t - this.f80mX[startIndex];
        float fraction = tInRange / xRange;
        float startY = this.f81mY[startIndex];
        float endY = this.f81mY[endIndex];
