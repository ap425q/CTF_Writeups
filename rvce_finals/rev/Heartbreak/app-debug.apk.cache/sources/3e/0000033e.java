...
public class MonotonicCurveFit extends CurveFit {
    ...
    public void getPos(double t, double[] v) {
        int n = this.f33mT.length;
        int dim = this.f34mY[0].length;
        if (this.mExtrapolate) {
            if (t <= this.f33mT[0]) {
                getSlope(this.f33mT[0], this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = this.f34mY[0][j] + ((t - this.f33mT[0]) * this.mSlopeTemp[j]);
                }
                return;
            } else if (t >= this.f33mT[n - 1]) {
                getSlope(this.f33mT[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = this.f34mY[n - 1][j2] + ((t - this.f33mT[n - 1]) * this.mSlopeTemp[j2]);
                }
                return;
            }
        } else if (t <= this.f33mT[0]) {
            for (int j3 = 0; j3 < dim; j3++) {
                v[j3] = this.f34mY[0][j3];
            }
            return;
        } else if (t >= this.f33mT[n - 1]) {
            for (int j4 = 0; j4 < dim; j4++) {
                v[j4] = this.f34mY[n - 1][j4];
            }
            return;
        }
        for (int i = 0; i < n - 1; i++) {
            if (t == this.f33mT[i]) {
                for (int j5 = 0; j5 < dim; j5++) {
                    v[j5] = this.f34mY[i][j5];
                }
            }
            if (t < this.f33mT[i + 1]) {
                double h = this.f33mT[i + 1] - this.f33mT[i];
                double x = (t - this.f33mT[i]) / h;
                for (int j6 = 0; j6 < dim; j6++) {
                    double y1 = this.f34mY[i][j6];
                    double y2 = this.f34mY[i + 1][j6];
    ...
    public void getPos(double t, float[] v) {
        ...
        if (this.mExtrapolate) {
            if (t <= this.f33mT[0]) {
                getSlope(this.f33mT[0], this.mSlopeTemp);
                for (int j = 0; j < dim; j++) {
                    v[j] = (float) (this.f34mY[0][j] + ((t - this.f33mT[0]) * this.mSlopeTemp[j]));
                }
                return;
            } else if (t >= this.f33mT[n - 1]) {
                getSlope(this.f33mT[n - 1], this.mSlopeTemp);
                for (int j2 = 0; j2 < dim; j2++) {
                    v[j2] = (float) (this.f34mY[n - 1][j2] + ((t - this.f33mT[n - 1]) * this.mSlopeTemp[j2]));
                }
                return;
            }
        } else if (t <= this.f33mT[0]) {
            for (int j3 = 0; j3 < dim; j3++) {
                v[j3] = (float) this.f34mY[0][j3];
            }
            return;
        } else if (t >= this.f33mT[n - 1]) {
            for (int j4 = 0; j4 < dim; j4++) {
                v[j4] = (float) this.f34mY[n - 1][j4];
            }
            return;
        }
        for (int i = 0; i < n - 1; i++) {
    ...
    public double getPos(double t, int j) {
        ...
        if (this.mExtrapolate) {
            if (t <= this.f33mT[0]) {
                return this.f34mY[0][j] + ((t - this.f33mT[0]) * getSlope(this.f33mT[0], j));
            }
            if (t >= this.f33mT[n - 1]) {
                return this.f34mY[n - 1][j] + ((t - this.f33mT[n - 1]) * getSlope(this.f33mT[n - 1], j));
            }
        } else if (t <= this.f33mT[0]) {
            return this.f34mY[0][j];
        } else {
            if (t >= this.f33mT[n - 1]) {
                return this.f34mY[n - 1][j];
            }
        }
        for (int i = 0; i < n - 1; i++) {
            if (t == this.f33mT[i]) {
                return this.f34mY[i][j];
            }
            if (t < this.f33mT[i + 1]) {
                double h = this.f33mT[i + 1] - this.f33mT[i];
                double x = (t - this.f33mT[i]) / h;
                double y1 = this.f34mY[i][j];
                double y2 = this.f34mY[i + 1][j];
    ...
    public void getSlope(double t, double[] v) {
        ...
        for (int i = 0; i < n - 1; i++) {
            if (t2 <= this.f33mT[i + 1]) {
                double h = this.f33mT[i + 1] - this.f33mT[i];
                double x = (t2 - this.f33mT[i]) / h;
                for (int j = 0; j < dim; j++) {
                    double y1 = this.f34mY[i][j];
                    double y2 = this.f34mY[i + 1][j];
                    double t1 = this.mTangent[i][j];
