...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f105mX;

    /* renamed from: mY */
    private float[] f106mY;
    ...
    private void initPath(Path path) {
        ...
        this.f105mX = new float[numPoints];
        this.f106mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            float distance = (i * pathLength) / (numPoints - 1);
            pathMeasure.getPosTan(distance, position, null);
            this.f105mX[i] = position[0];
            this.f106mY[i] = position[1];
        }
        if (Math.abs(this.f105mX[0]) > 1.0E-5d || Math.abs(this.f106mY[0]) > 1.0E-5d || Math.abs(this.f105mX[numPoints - 1] - 1.0f) > 1.0E-5d || Math.abs(this.f106mY[numPoints - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f105mX[0] + "," + this.f106mY[0] + " end:" + this.f105mX[numPoints - 1] + "," + this.f106mY[numPoints - 1]);
        }
        float prevX = 0.0f;
        ...
        while (i2 < numPoints) {
            ...
            float x = this.f105mX[componentIndex];
            if (x < prevX) {
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f105mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f105mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f105mX[endIndex] - this.f105mX[startIndex];
        if (xRange == 0.0f) {
            return this.f106mY[startIndex];
        }
        float tInRange = t - this.f105mX[startIndex];
        float fraction = tInRange / xRange;
        float startY = this.f106mY[startIndex];
        float endY = this.f106mY[endIndex];
