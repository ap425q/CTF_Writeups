...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f65mX;

    /* renamed from: mY */
    private final float[] f66mY;
    ...
    public PathInterpolatorApi14(Path path) {
        ...
        this.f65mX = new float[numPoints];
        this.f66mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            float distance = (i * pathLength) / (numPoints - 1);
            pathMeasure.getPosTan(distance, position, null);
            this.f65mX[i] = position[0];
            this.f66mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f65mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f65mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
