...
public class MotionPaths implements Comparable<MotionPaths> {
    ...
    void initCartesian(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float f5 = startTimePoint.f29x;
        float startCenterX = f5 + (f2 / 2.0f);
        float position2 = startTimePoint.f30y;
        float startCenterY = position2 + (f4 / 2.0f);
        float endCenterX = endTimePoint.f29x + (f / 2.0f);
        float endCenterY = endTimePoint.f30y + (f3 / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f29x = (int) ((f5 + (pathVectorX * position)) - ((scaleX * scaleWidth) / 2.0f));
        this.f30y = (int) ((position2 + (pathVectorY * position)) - ((scaleY * scaleHeight) / 2.0f));
        ...
        this.mMode = 2;
        this.f29x = (int) (((startTimePoint.f29x + (pathVectorX * dxdx)) + (pathVectorY * dxdy)) - ((scaleX * scaleWidth) / 2.0f));
        this.f30y = (int) (((startTimePoint.f30y + (pathVectorX * dydx)) + (pathVectorY * dydy)) - ((scaleY * scaleHeight) / 2.0f));
    ...
    void initScreen(int parentWidth, int parentHeight, KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        this.mMode = 3;
        if (!Float.isNaN(c.mPercentX)) {
            this.f29x = (int) (c.mPercentX * ((int) (parentWidth - this.width)));
        }
        if (!Float.isNaN(c.mPercentY)) {
            this.f30y = (int) (c.mPercentY * ((int) (parentHeight - this.height)));
        }
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    void initPath(KeyPosition c, MotionPaths startTimePoint, MotionPaths endTimePoint) {
        ...
        float startCenterX = (startTimePoint.width / 2.0f) + startTimePoint.f29x;
        float startCenterY = startTimePoint.f30y + (startTimePoint.height / 2.0f);
        float endCenterX = endTimePoint.f29x + (endTimePoint.width / 2.0f);
        float endCenterY = endTimePoint.f30y + (endTimePoint.height / 2.0f);
        float pathVectorX = endCenterX - startCenterX;
        float pathVectorY = endCenterY - startCenterY;
        this.f29x = (int) ((f + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f30y = (int) ((f3 + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.width = (int) (f2 + (scaleX * scaleWidth));
        ...
        this.mMode = 1;
        float f4 = (int) ((startTimePoint.f29x + (pathVectorX * path)) - ((scaleX * scaleWidth) / 2.0f));
        this.f29x = f4;
        float f5 = (int) ((startTimePoint.f30y + (pathVectorY * path)) - ((scaleY * scaleHeight) / 2.0f));
        this.f30y = f5;
        this.f29x = f4 + normalX;
        this.f30y = f5 + normalY;
        this.mKeyFrameEasing = Easing.getInterpolator(c.mTransitionEasing);
        this.mPathMotionArc = c.mPathMotionArc;
    }
    ...
    public void different(MotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        ...
        int c2 = c + 1;
        mask[c] = mask[c] | diff(this.f29x, points.f29x) | arcMode;
        int c3 = c2 + 1;
        mask[c2] = mask[c2] | diff(this.f30y, points.f30y) | arcMode;
        int c4 = c3 + 1;
    ...
    public void fillStandard(double[] data, int[] toUse) {
        float[] set = {this.position, this.f29x, this.f30y, this.width, this.height, this.mPathRotate};
        int c = 0;
        for (int i = 0; i < toUse.length; i++) {
