...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f82mX = new float[numPoints];
        this.f83mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            float distance = (i * pathLength) / (numPoints - 1);
            pathMeasure.getPosTan(distance, position, null);
            this.f82mX[i] = position[0];
            this.f83mY[i] = position[1];
        }
        if (Math.abs(this.f82mX[0]) > 1.0E-5d || Math.abs(this.f83mY[0]) > 1.0E-5d || Math.abs(this.f82mX[numPoints - 1] - 1.0f) > 1.0E-5d || Math.abs(this.f83mY[numPoints - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f82mX[0] + "," + this.f83mY[0] + " end:" + this.f82mX[numPoints - 1] + "," + this.f83mY[numPoints - 1]);
        }
        float prevX = 0.0f;
        int componentIndex = 0;
        int i2 = 0;
        while (i2 < numPoints) {
            float[] fArr = this.f82mX;
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f82mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f82mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        ...
        if (xRange == 0.0f) {
            return this.f83mY[startIndex];
        }
        float tInRange = t - fArr[startIndex];
        float fraction = tInRange / xRange;
        float[] fArr2 = this.f83mY;
